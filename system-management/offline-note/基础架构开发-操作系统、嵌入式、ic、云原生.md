...
所有键盘、显示器、命令行、应用程序窗口都可视为终端。
其中键盘属于输入控制，也有部分键盘拥有小屏幕可以作为输出显示。
而显示器就是物理输出终端，命令行和应用程序窗口都属于虚拟输出终端，其中命令行相比其他终端会拥有更强大的控制操作权限。

从键盘输入字符到显示在命令行上，需要经过：
1. 键盘发送信号给键盘驱动程序；
2. UART驱动程序（对应键盘）处理后发给tty串口程序；
3. tty串口程序直接上交给line discipline行规程驱动程序；
4. 行规程可进行处理后将最终情况发给TTY驱动程序，例如普通字母字符就存储到buffer缓存里后再返回，如果是退格会在buffer缓存里删除后继续返回退格字符，若是遇到回车就会将buffer缓存里的所有字符作为命令发给shell处理，等接受到结果则返回；
5. shell若接到数据则执行相应命令，例如cd字符代表要获取当前路径下的所有文件雨文件夹，那就进行获取后将结果返回；
6. TTY串口程序将行规程返回的数据传递给用户进程，用户进程可以将结果输出到应用程序窗口上；
7. 另一边行规程驱动程序也会将处理结果一路从UART驱动程序发回给键盘设备，此时如果键盘也带有屏幕则可以接收到并输出字符。
* UART驱动、行规程、TTY驱动都属于内核程序。
* 行规程驱动程序可以设置为原始模式，这样就不会对物理设备发送的数据进行默认处理或存储字符，而是直接传递原始数据给用户进程，例如你接入了一个GPS设备，设置为原始模式后，你的应用程序就可以接受到这个GPS从串口发过来的原始数据，并根据设备厂家提供的资料手册进行处理。

## 关于硬件中的物理设备如何对应到设备节点

设备节点即看起来像是一个路径`/dev/tty0`，会对应到实际的物理设备上，这些物理设备是通过串口连接的。
一般开发板厂家都已经做好支持，不需要开发者从头开发驱动程序或者处理串口连接，而只需要做自己的应用程序来使用这些设备节点即可。
如果是出厂板子没有做好串口支持，也会有默认的驱动程序设备树配置好但处于未启用的状态，需要开发者手动使用配置文件覆盖启用，此时就可以开发应用程序了。
如果需要开发驱动程序，则需要了解：
物理设备在硬件是至少有两个引脚连接，对应数据输入和输出，而在系统内核设备树文件中就配置好了每个引脚对应硬件引脚信息，如果需要开发驱动程序，只需要将这些配置好的引脚信息名称作为引脚引入到驱动程序对应的设备树文件里即可，而无需自己从头根据硬件来配置引脚；接着将串口名称和该配置文件里的别名对应起来就好。
